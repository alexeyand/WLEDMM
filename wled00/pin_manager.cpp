...
String PinManagerClass::getPinSpecialText(int gpio) {  // special purpose PIN info
  ...
  if (gpio == i2c_sda)  return(F("(default) I2C SDA")); //WLEDMM simplify i2C
  if (gpio == i2c_scl)  return(F("(default) I2C SCL")); //WLEDMM simplify i2C
  if (gpio == spi_sclk) return(F("(default) SPI SLK  / SCK")); //WLEDMM simplify i2C
  if (gpio == spi_mosi) return(F("(default) SPI PICO / MOSI")); //WLEDMM simplify i2C
  if (gpio == spi_miso) return(F("(default) SPI POCI / MISO")); //WLEDMM simplify i2C
